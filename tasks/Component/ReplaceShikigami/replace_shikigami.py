    ...
    def switch_shikigami_class(self, shikigami_class: ShikigamiClass = ShikigamiClass.N):
        ...
        while 1:
            ...
            if self.appear(check_click, interval=1):
                logger.info(f'Wait until {check_click.name} position stable')
                if self.wait_until_pos_stable(check_click, stable_time=0.8, timeout=3):
                    logger.info(f'{check_click.name} position already stabled, click')
